...
В качестве исходных данных даются стартовые адреса в памяти, в которых хранятся массивы, количество элементов в массивах и формула для требуемых вычислений (Вычисления производятся над массивами поэлементно). 
...
Доступ к массивам (чтение, запись) должен выполняться из памяти. Предполагайте, что в запускаем окружение размер векторного регистра составляет 256 бит, максимальный размер одного элемента 64 бита, в массивы состоят 32-х битных чисел
